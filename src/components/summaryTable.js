...
class SummaryTable extends React.Component {
  ...
  handleSearch = event => {
    ...
    if (text) {
      ...
      tmpData = tmpData.filter(
        item =>
          item.country.name &&
          item.country.name.toLowerCase().includes(text.toLowerCase())
      )
      this.setState({
        filterText: text,
        filterData: tmpData,
  ...
  handlePress = event => {
    if (event.keyCode == 13) {
      //for mobile if "ENTER" buton pressed
      console.log("asdf")
      event.preventDefault()
      event.target.blur()
    }
  ...
  render() {
    ...
    return (
      <div className="md:mx-32 mx-8 my-4">
        <div className="w-full flex justify-between">
          <h1 className="underline text-md sm:text-xl mb-1">
            {this.props.title}
          </h1>
          <input
            className="shadow appearance-none border rounded w-full sm:w-3/12 py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline"
            type="search"
